...
class User:
    ...
    def Fetch(self) -> list:
        ...
        phone = data[0][6]
        global users
        data = [id_,self.login,self.password,surname,name,secondname,birth_date,email,phone]
        logging.warning(f"successfully fetched data:{id_},{self.login},{self.password},{surname},{name},{secondname},{birth_date},{email},{phone}")
        users.append(data)
        #print(users[-1])
        return data
    ...
    def Auth(self) -> bool:
        ...
        if len(user) == 0:
            ...
            if len(record) == 0:
                ...
                Ans = tkinter.Label(callback,text = f"Здравствуйте,{self.login}").grid(row=0,column=0,columnspan=3)
                ed = tkinter.Button(callback,text = "Edit profile", command = lambda: self.EditGUI()).grid(row=1,column=0)
    ...
    def NewPasswd(self, id_, old,new,repeat):
        try:
            if (old != new) and (new == repeat) and (len(new) <= 20):
                mydb=conn.connect(host=host,database=dbname,user=login,password=password)
                c = mydb.cursor()
                c.execute(f"UPDATE login SET password = '{new}' WHERE id = {id_}")
                logging.warning(f"password on {id_} changed from {old} to {new}")
            else:
    ...
    def SubmitChanges(self,old,new):                       ## [id_,self.login,self.password,surname,name,secondname,birth_date,email,phone]
        try:
            mydb=conn.connect(host=host,database=dbname,user=login,password=password)
            c = mydb.cursor()
        except Exception as e:
    ...
    def EditGUI(self):
        ...
        passwd_l = tkinter.Label(GUI,text="Пароль текущий:").grid(row=1,column=0)
        passwd_l_N = tkinter.Label(GUI,text="Пароль Новый:").grid(row=2,column=0)
        passwd_l_N_R = tkinter.Label(GUI,text="Повторите новый пароль").grid(row=3,column=0)
        ...
        secondname_l = tkinter.Label(GUI,text="Отчество").grid(row=6,column=0)
        ...
        B = tkinter.Button(GUI,text="Apply",command = lambda: self.SubmitChanges(data,[data[0]] + [x.get() for x in form]) if (len(passwd_N.get()) == 0) else (self.NewPasswd(data[0],passwd.get(),passwd_N.get(),passwd_N_R.get()))).grid(row=10,column=1)
        data = self.Fetch()
